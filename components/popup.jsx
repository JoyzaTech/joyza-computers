        ...
        <>
            ...
            <div className="p-6 rounded flex flex-row items-center">
                <div className="bg-[#3A3A3A] shadow-lg max-w-md w-full relative flex flex-col items-center text-center m-3 p-3 rounded-xl">
                    ...
                    <Image src={info.images[0]} alt='popup info' width='300' height='300' />
                    ...
                    <div className="purcahse-container flex flex-row p-2">
                        ...
                        <div className="bg-[#5D5D5D] w-[2vw] h-[2vw] rounded-full pt-1 text-white my-2"><p>{quantity}</p></div>
                        <button onClick={() => setQuantity(quantity + 1)} className="m-2"><CgMathPlus /></button>
                        <button
                        className="bg-[#0071C7] hover:bg-[#005FA8] p-2 rounded-lg"
                        onClick={() => {
                        onAdd(info.name, info.price, quantity);
                        onClick(); // This will close the popup
                        }}
                        >
                        Add to cart
                        </button>
                ...
                <div className="bg-[#3A3A3A] shadow-lg max-w-md w-full relative flex flex-col items-center text-center m-3 p-3 rounded-xl">
                    <button className="absolute top-2 right-2 text-white text-xl hover:text-gray-400" onClick={onClick}>X</button>
                    ...
                    {info.features.map((feature) => {
                        return(
                            <li className="text-[1vw]" key={feature}>{feature}</li>
                        )
                    })}
                    <hr className="bg-[#5D5D5D] w-full h-[4px] border-none my-4"/>
                    ...
                    {Object.values(info.specifications).map((specification) => {
                        return(
                            <li className="text-[1vw]" key={specification}>{specification}</li>
                        )
                    })}
                </div>
            </div>
